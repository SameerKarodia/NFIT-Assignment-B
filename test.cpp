...
int main() {
    ...
    const double wavenum1 = (2 * PI) / wavelength1;
    ...
    const double wavenum2 = (2 * PI) / wavelength2;
    cout << wavelength2<<endl;
    cout << angfre2<<endl;
    cout << wavenum2<<endl;
    ...
    double x_max = 100*PI; // Maximum distance or time for plotting
    double x_step = PI/100; // Step size
    ...
    for (double x = 0; x <= x_max; x += x_step) {
        ...
        double D1 = (test == "t1") ? amp1 * sin(wavenum1 * x + angfre1 * t1 + iniphase1)
            : amp1 * sin(wavenum1 * x1 + angfre1 * x + iniphase1);

        double D2 = (test2 == "t2") ? amp2 * sin(wavenum2 * x + angfre2 * t2 + iniphase2)
            : amp2 * sin(wavenum2 * t2 + angfre2 * x + iniphase2);
    ...
    if (gnuplotPipe) {
        ...
        fprintf(gnuplotPipe, "plot 'signals.dat' using 1:2 title 'Signal 1' with lines linecolor rgb 'red', ");
        fprintf(gnuplotPipe, "'signals.dat' using 1:3 title 'Signal 2' with lines linecolor rgb 'blue', ");
        fprintf(gnuplotPipe, "'signals.dat' using 1:4 title 'Sum of Signals' with lines linecolor rgb 'orange'\n");
        fflush(gnuplotPipe);
        system("pause");
        _pclose(gnuplotPipe);
    }
